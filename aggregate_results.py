...
def aggregate_results(results_dir: Path, num_classes: int, mode: str, report_output_path: Path, stats_output_path: Path | None):
    ...
    if mode == 'cs':
        if stats_output_path is None:
            print("\nエラー: --mode cs の場合は --stats-out の指定が必須です。")
            return
...
if __name__ == "__main__":
    ...
    if args.num_classes is not None:
        ...
        try:
            ...
            raise ValueError(f"--configで指定されたファイルが見つかりません: {args.config}")
    
    if num_classes is None:
        raise ValueError("`--num-classes`引数、または`--config`ファイル内の`num_classes`の指定が必要です。")
