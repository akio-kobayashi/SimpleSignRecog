...
from sklearn.svm import SVC  # サポートベクターマシン（SVM）モデルを構築するために使用
...
def main(config: dict):
    """
    サポートベクターマシン（SVM）モデルの訓練と評価を行うメインの関数。
    ...
    for fold, (train_indices, test_indices) in enumerate(cv_splitter.split(X, y)):
        ...
        # --- 2c. SVMモデルの訓練 ---
        print("--- SVMモデルを訓練します ---")
        # SVCクラスでSVMモデルを定義します。kernel='rbf'は非線形な決定境界を作るための一般的なカーネルです。
